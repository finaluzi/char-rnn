...
local LSTM = require 'model.LSTM_oneHot2'
...
if opt.gpuid >= 0 and opt.opencl == 1 then
	...
	for t=1,opt.seq_length do
		real_x[t] = real_x[t]:cl()
	end
	real_x_temp = real_x_temp:cl()
end
...
function eval_split(split_index, max_batches)
    ...
    for i = 1,n do -- iterate over batches in the split
        ...
        for t=1,opt.seq_length do
			...
			rnn_state_temp = {}
			for i=1,#init_state do table.insert(rnn_state_temp, lst[i]) end
...
--    local _, loss = optim.rmsprop(feval, params, optim_state)
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local savefile = string.format('%s/%s_ep%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, epoch, train_loss)
